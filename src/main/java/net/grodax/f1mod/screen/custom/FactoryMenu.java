...
public class FactoryMenu extends AbstractContainerMenu {
    ...
    public FactoryMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        ...
        this.data = data;
        int gridStartX = 16;  // Change this to your measured X offset
        int gridStartY = 22;
        ...
        for (int row = 0; row < 5; row++) {
            for (int col = 0; col < 5; col++) {
                this.addSlot(new SlotItemHandler(blockEntity.itemHandler,
                        col + row * 5,
                        gridStartX + col * 18, // Spaces them exactly 18px apart
                        gridStartY + row * 18  // Spaces them exactly 18px apart
                ));
            }
        }

        // Add Output Slot (Slot 25)
        // Using a custom Slot class or overriding mayPlace
        this.addSlot(new SlotItemHandler(blockEntity.itemHandler,  25, 146, 55) {
            @Override
            public boolean mayPlace(ItemStack stack) {
                return false; // Prevents players from putting items in the output
            }
        });
        addDataSlots(data);
    }
    ...
    private static final int TE_INVENTORY_SLOT_COUNT = 26; //2 // must be the number of slots you have!
    @Override
